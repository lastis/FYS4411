...
the Slater matrix is changed, but by calculating the  from scratch each time,
we will certainly do unnecessary computations. This motivates the ratio
\eq{
  R = \frac{|\op D|_{\uparrow}^{new}}{|\op D|_{\uparrow}^{old}} 
  \cdot \frac{|\op D|_{\downarrow}^{new}}{|\op D|_{\downarrow}^{old}}
}
Every time we accept a new trial position, the i-th row of $\op D$ changes,
and we have to update the inverse.
\section{Results}
