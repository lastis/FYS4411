...
\equ{
  \psi_C = \prod_{i<j}^{4}\exp{\left(\frac{r_{ij}}{2(1+\beta r_{ij})}\right)} \label{psiC}
}
...
\eq{
  f_{ij} \equiv \frac{r_{ij}}{2(1+\beta r_{ij})}
}
...
by the differentiation, so
\eq{
  \frac{\partial\psi_C}{\partial x_k}=\prod_{i,j\neq k} g_{ij}
    \frac{\partial}{\partial x_k}
    \left[ \prod_{i<k} g_{ik} + \prod_{i>k} g_{ki} \right]
}
