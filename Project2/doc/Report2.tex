...
differentiation. When we split the expression for $i<k$ and $k>i$,
and 
\equ{
  \frac{\partial\psi_C}{\partial x_k}&=\prod_{i,j\neq k} g_{ij}
    \frac{\partial}{\partial x_k}
    \left[ \prod_{i<k} g_{ik} \cdot \prod_{i>k} g_{ki} \right]\label{DpsiC}\\
    &=\prod_{i,j\neq k} g_{ij}
      ...
      \nonumber\\
    &=\prod_{i,j\neq k} g_{ij}
      \left[\prod_{i>k} g_{ki} \sum_{i<k} g_{ik}' \prod_{p\neq i} g_{pi}
      + \prod_{i<k} g_{ik} \sum_{i>k} g_{ki}' \prod_{q\neq i} g_{iq} \right]
      \nonumber\\
    &=\prod_{i<j} g_{ij}
      \left[\frac{1}{\prod_{i<k} g_{ik}} \sum_{i<k} g_{ik}' \prod_{p\neq i} g_{pi}
      + \frac{1}{\prod_{i>k} g_{ki}} \sum_{i>k} g_{ki}' \prod_{p\neq i} g_{pi} \right]
      \nonumber\\
    &=\psi_C
      \left[\sum_{i<k} \frac{g_{ik}'}{g_{ik}}
      +\sum_{i>k} \frac{g_{ki}'}{g_{ki}} \right]
      \nonumber
}
Here we factorize the wavefunction outside the expression, 
and notice that the only part that doesn't cancel is the $ik$-th and $ki$-th
in the sums.
